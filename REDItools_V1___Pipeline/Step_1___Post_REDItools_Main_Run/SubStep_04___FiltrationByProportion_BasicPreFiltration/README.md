# SubStep 04: Filtration By Proportion - Basic Pre-Filtration

## Protocol For SubStep 4 Filtration Via Proportion (Including Basic Filtration):

1) For the `ipynb` staring with `Part_1_And_Part_2` you will need the following:

| Variable | Description |
|-----------------|-----------------|
| `file_path` | Give the full path to the SubStep 3 Part 2 `tsv` |

The `ipynb` will find the folder `SubStep_3___MergingIntoOneTable` is contained in and create a folder`SubStep_4___FilterationViaProportion___BasicPreFilteration` with each part (Part 1 and Part 2) in a subfolder of `SubStep_4___FilterationViaProportion___BasicPreFilteration` for all the outputs.
