...
REDITs_Cloned_Directory <- "/path/to/cloned/REDITs/repository/REDITs/"

### Read the data from the basic filtered TSV file
data_file <- "/path/to/SubStep_4___FilterationViaProportion___BasicPreFilteration/Part_5___MergingPivotedTables_BasicPreFiltered_REDITsInput/Output_File.tsv"
...
grandparent_directory <- dirname(dirname(dirname(data_file)))

### Define the new subfolder name
subfolder_name <- "SubStep_06___REDITs_Main__Using_REDIT_LLR.R"
...
## Section 4: Source the REDIT_LLR.R file
...
output_directory <- file.path(parent_directory, "Part_1___REDITs_Main_Using_Basic_PreFiltered_File")
...
csv_file <- file.path(output_directory, "REDITs_Results_OnBasicPreFilteredData.csv")
write.csv(final_result_df, file = csv_file, row.names = FALSE)

### Write the data frame to a TSV file
tsv_file <- file.path(output_directory, "REDITs_Results_OnBasicPreFilteredData.tsv")
